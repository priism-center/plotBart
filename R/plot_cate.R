...
plot_CATE <- function(.model, type = c('histogram', 'density'), ci_80 = FALSE, ci_95 = FALSE, reference = NULL, .mean = FALSE, .median = FALSE){
  ...
  if(.model$estimand == 'ate'){
    .title <- "Posterior of Average Treatment Effect"
  }
  if(.model$estimand == 'att'){
    .title <- "Posterior of Average Treatment Effect of the Treated"
  }
  if(.model$estimand == 'atc'){
    .title <- "Posterior of Average Treatment Effect of the Control"
  }
  # calculate stats
